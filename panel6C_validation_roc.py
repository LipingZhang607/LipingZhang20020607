...
fig.suptitle(
    "Panel 7C: Independent Validation of Top10 SHAP Genes\n"
    "GSE254176 (SLE expression consistency) | GSE162577 (SLE vs Control + ROC)",
    fontsize=get_font_size("title"), fontweight="bold", y=1.01
)

plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "Panel6C.pdf"), bbox_inches="tight", dpi=300)
plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "Panel6C.png"), bbox_inches="tight", dpi=300)
plt.close(fig)
