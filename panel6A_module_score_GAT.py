...
# -*- coding: utf-8 -*-
"""
Panel 6A: GSE189050 独立验证 - GAT基因版本
- 用 GAT Top10 标记基因计算每个细胞的模块评分
- 验证GAT识别的敏感细胞在SLE患者中是否显著扩增
- 确认模块评分富集于特定免疫细胞亚群
"""
...
OUT_DIR    = "/home/h3033/statics/GEO_data/GSE/figure6/output/panel6A_GAT"
...
# ── 2. 读取 GAT Top10 标记基因 ─────────────────────────────────────────────────
print("[2/6] 读取 GAT Top10 标记基因...")
df_markers = pd.read_csv(GAT_FILE)
gat_top10 = df_markers.sort_values("importance_combined", ascending=False)["feature"].head(10).tolist()
print(f"      GAT Top10 genes: {gat_top10}")

# 只保留在当前数据集中存在的基因
available = [g for g in gat_top10 if g in adata.var_names]
missing   = [g for g in gat_top10 if g not in adata.var_names]
print(f"      GAT Top10 基因中: {len(available)} 个可用, {len(missing)} 个缺失")
if missing:
    print(f"      缺失基因: {missing}")

# ── 3. 计算模块评分 ───────────────────────────────────────────────────────────
print("[3/6] 计算模块评分 (sc.tl.score_genes)...")
sc.tl.score_genes(adata, gene_list=available, score_name="gat_score", use_raw=False)
print(f"      评分范围: [{adata.obs['gat_score'].min():.3f}, {adata.obs['gat_score'].max():.3f}]")

# 保存带评分的元数据
adata.obs[["classification", "coarse_cell_type", "fine_cell_type",
           "clusters_annotated", "gat_score"]].to_csv(
    os.path.join(OUT_DIR, "cell_scores_GAT.csv")
)

# ── 4. 统计检验：SLE vs Control ───────────────────────────────────────────────
print("[4/6] 统计检验...")

ctrl_scores = adata.obs.loc[adata.obs["classification"] == "Control",    "gat_score"]
inact_scores = adata.obs.loc[adata.obs["classification"] == "SLE INACT", "gat_score"]
act_scores   = adata.obs.loc[adata.obs["classification"] == "SLE ACT",   "gat_score"]
...
stats_df.to_csv(os.path.join(OUT_DIR, "statistics_GAT.csv"), index=False)

# ── 5. 各细胞亚群平均评分 ─────────────────────────────────────────────────────
celltype_scores = (adata.obs.groupby("fine_cell_type")["gat_score"]
                   .agg(["mean", "median", "std"])
                   .sort_values("mean", ascending=False))
celltype_scores.to_csv(os.path.join(OUT_DIR, "celltype_scores_GAT.csv"))
print("[5/6] 细胞亚群评分排名:")
...
adata.obs["is_gat_sensitive"] = (adata.obs["gat_score"] > 0).astype(int)
...
if donor_col:
    prop_df = (
        ...
        .agg(
            ...
            proportion    =("is_gat_sensitive",   "mean"),
            n_cells       =("is_gat_sensitive",   "count"),
        )
        .reset_index()
    )
    prop_df = prop_df[prop_df["classification"].isin(DISEASE_ORDER)]
    prop_df.to_csv(os.path.join(OUT_DIR, "donor_gat_proportion.csv"), index=False)
...
for grp in DISEASE_ORDER:
    ...
    ax1.scatter(
        ...
        c=DISEASE_COLORS[grp], s=get_marker_size("small"), alpha=0.4, label=grp, rasterized=True
    )
ax1.set_title("UMAP — Disease Status", fontsize=get_font_size("title"), fontweight="bold")
ax1.set_xlabel("UMAP1"); ax1.set_ylabel("UMAP2")
ax1.legend(markerscale=5, frameon=False, fontsize=get_font_size("tick"))
...
sc_plot = ax2.scatter(
    adata.obsm["X_umap"][:, 0],
    adata.obsm["X_umap"][:, 1],
    c=adata.obs["gat_score"],
    cmap="RdYlBu_r", s=get_marker_size("small"), alpha=0.5, vmin=-0.5, vmax=2.0, rasterized=True
)
plt.colorbar(sc_plot, ax=ax2, shrink=0.7, label="GAT Module Score")
ax2.set_title("UMAP — GAT Module Score", fontsize=get_font_size("title"), fontweight="bold")
...
for i, ct in enumerate(cell_types):
    ...
    ax3.scatter(
        adata.obsm["X_umap"][idx, 0],
        adata.obsm["X_umap"][idx, 1],
        c=[palette[i]], s=get_marker_size("small"), alpha=0.4, label=ct, rasterized=True
    )
ax3.set_title("UMAP — Cell Type", fontsize=get_font_size("title"), fontweight="bold")
ax3.set_xlabel("UMAP1"); ax3.set_ylabel("UMAP2")
ax3.legend(markerscale=5, frameon=False, fontsize=get_font_size("legend"), ncol=1, loc="lower left")
...
for xi, xj, p in [(0, 1, p_ctrl_inact), (0, 2, p_ctrl_act), (1, 2, p_inact_act)]:
    ...
    ax4.plot([xi, xj], [y_top, y_top], "k-", linewidth=get_line_width("plot"))
    ax4.text((xi+xj)/2, y_top+0.02, sig_label(p), ha="center", fontsize=get_font_size("title"))

ax4.set_xticks([0, 1, 2])
ax4.set_xticklabels(DISEASE_ORDER, rotation=15, ha="right")
ax4.set_ylabel("GAT Module Score")
ax4.set_title("GAT Module Score by Disease Status", fontsize=get_font_size("title"), fontweight="bold")

# -- 6D-new: Boxplot -- GAT sensitive cell proportion per donor by disease ------
...
if prop_df is not None and len(groups_prop) > 0:
    ...
    box_data  = [groups_prop[g] * 100 for g in present_groups]
    positions = list(range(len(present_groups)))

    bp = ax_box.boxplot(
        box_data, positions=positions, widths=get_marker_size("small"),
    ...
    for i, (grp, vals) in enumerate(zip(present_groups, box_data)):
        ...
        ax_box.scatter(jitter, vals, s=18, color=DISEASE_COLORS[grp],
                       edgecolors="white", linewidths=get_marker_size("small"), zorder=3, alpha=0.85)
    ...
    for level, (xi, xj, p_val) in enumerate(
        ...
        ax_box.plot([xi, xi, xj, xj],
                    ...
                    "k-", linewidth=get_line_width("plot"))
        ax_box.text((xi + xj) / 2, y_top + h_step * 0.05,
                    _sig(p_val), ha="center", va="bottom", fontsize=get_font_size("title"))

    ax_box.set_xticks(positions)
    ax_box.set_xticklabels(present_groups, rotation=15, ha="right", fontsize=get_font_size("tick"))
    ax_box.set_ylabel("GAT-sensitive cells (%)", fontsize=get_font_size("title"))
    ax_box.set_title("GAT-Sensitive Cell Proportion\nper Donor by Disease Status",
                     fontsize=get_font_size("title"), fontweight="bold")
    ax_box.text(0.98, 0.02, "P: cell-level MWU",
                transform=ax_box.transAxes, fontsize=get_font_size("legend"),
    ...
    ax_box.text(0.5, 0.5, "No donor info available",
                ...
                fontsize=get_font_size("tick"), color="gray")
    ax_box.set_title("GAT-Sensitive Cell Proportion", fontsize=get_font_size("title"), fontweight="bold")
...
ax5.errorbar(
    ...
    xerr=ct_order["std"].values, fmt="none", color="black", capsize=2, linewidth=get_line_width("plot")
)
ax5.set_yticks(range(len(ct_order)))
ax5.set_yticklabels(ct_order.index, fontsize=get_font_size("tick"))
ax5.axvline(0, color="gray", linestyle="--", linewidth=get_line_width("plot"))
ax5.set_xlabel("Mean GAT Module Score")
ax5.set_title("GAT Score by Cell Subtype (fine_cell_type)", fontsize=get_font_size("title"), fontweight="bold")

fig.suptitle("Panel 6A: GAT Module Score Validation in GSE189050",
             fontsize=get_font_size("title"), fontweight="bold", y=1.01)

out_path = os.path.join(OUT_DIR, "Panel6A_GAT.pdf")
plt.tight_layout()
fig.savefig(out_path, bbox_inches="tight", dpi=300)
plt.tight_layout()
fig.savefig(out_path.replace(".pdf", ".png"), bbox_inches="tight", dpi=300)
plt.close()
print(f"\n完成！输出: {OUT_DIR}/Panel6A_GAT.pdf / .png")
