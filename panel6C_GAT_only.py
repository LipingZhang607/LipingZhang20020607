...
OUT_DIR       = "/home/h3033/statics/GEO_data/GSE/figure6/output/panel6C_GAT_only"
...
if adata_254 is not None:
    ...
    for gene in GAT_TOP10:
        ...
        dot_records.append({
            ...
            "importance": GAT_VALS.get(gene, 0),
        })

dot_df = pd.DataFrame(dot_records).set_index("gene")
heatmap_df = pd.DataFrame(sample_mean_records).pivot(index="sample", columns="gene", values="mean_expr").reindex(columns=GAT_TOP10)
...
if adata_162 is not None:
    ...
    # Composite score
    avail = [g for g in GAT_TOP10 if g in adata_162.var_names]
    if avail:
        ...
        roc_results["Composite_GAT"] = {"fpr": fpr, "tpr": tpr, "auc": auc(fpr, tpr)}

stat_df = pd.DataFrame(stat_records).sort_values("p_value")

# 保存结果
dot_df.to_csv(os.path.join(OUT_DIR, "GAT_GSE254176_dot_stats.csv"))
heatmap_df.to_csv(os.path.join(OUT_DIR, "GAT_GSE254176_sample_heatmap.csv"))
stat_df.to_csv(os.path.join(OUT_DIR, "GAT_GSE162577_stats.csv"), index=False)
auc_df = pd.DataFrame([{"gene": g, "auc": v["auc"]} for g, v in roc_results.items()])
auc_df.to_csv(os.path.join(OUT_DIR, "GAT_GSE162577_auc.csv"), index=False)
...
ax.set_title("GSE254176 (SLE, n=19)\nGAT Genes Cross-sample Expression", fontweight='bold')
...
if not dot_df.empty:
    ...
    sc_dot = ax.scatter(np.zeros(len(genes_ordered)), y_pos, s=sizes * 3,
                        c=colors, cmap="YlOrRd", vmin=0, vmax=colors.max(),
...
ax.set_title("GSE254176 (SLE)\nGAT Genes Expression", fontweight='bold')
...
if roc_results:
    for gene, res in roc_results.items():
        if gene.startswith('Composite'):
            ax.plot(res['fpr'], res['tpr'], 'r-', lw=2.5,
                    label=f"{gene} (AUC={res['auc']:.2f})", zorder=10)
        else:
            color = GAT_COLORS.get(gene, 'gray')
...
ax.set_title("GSE162577\nGAT Genes: SLE vs Control", fontweight='bold')
...
if roc_results:
    ...
    colors_s = [GAT_COLORS.get(g, "gray") for g in genes_s]
    y_pos = np.arange(len(genes_s))
    ax.barh(y_pos, auc_s, color=colors_s, alpha=0.8, edgecolor="gray", linewidth=0.4)
    if "Composite_GAT" in roc_results:
        ax.axvline(roc_results["Composite_GAT"]["auc"], color="red", lw=2,
                   linestyle="-", label=f"Composite AUC={roc_results['Composite_GAT']['auc']:.2f}")
...
if not stat_df.empty:
    ...
    n_total = len(stat_df)
    composite_auc = roc_results.get("Composite_GAT", {}).get("auc", 0)
...
fig.suptitle('Panel 6C: GAT Genes Independent Validation\n'
             'GSE254176 (SLE consistency) | GSE162577 (SLE vs Control + ROC)',
             fontsize=get_font_size("title"), fontweight='bold', y=0.98)

plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, 'Panel6C_GAT_only.pdf'), bbox_inches='tight', dpi=300)
plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, 'Panel6C_GAT_only.png'), bbox_inches='tight', dpi=300)
plt.close(fig)

print(f"\n完成！输出文件:")
print(f"  图表: {OUT_DIR}/Panel6C_GAT_only.pdf/png")
print(f"  数据: {OUT_DIR}/GAT_*.csv")
print(f"\nComposite AUC: {roc_results.get('Composite_GAT', {}).get('auc', 0):.3f}")
