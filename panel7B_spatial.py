...
数据说明:
  ...
  注意: 当前 GSE186476_RAW.tar 文件疑似下载不完整（内容为HTML），
        请重新从GEO下载后解压到 DATA_DIR，脚本会自动处理。
        解压命令示例: tar -xf GSE186476_RAW.tar -C DATA_DIR/
"""
...
fig_width, fig_height = get_figure_size(n_panels=13, layout='double')
...
fig1.suptitle("Panel 7B (scRNA-seq): Top 10 SHAP Genes in GSE186476", fontsize=get_font_size("title"), fontweight="bold")
fig1.savefig(os.path.join(OUT_DIR, "Panel6B_scrna.pdf"), bbox_inches="tight", dpi=300)
fig1.savefig(os.path.join(OUT_DIR, "Panel6B_scrna.png"), bbox_inches="tight", dpi=300)
plt.close(fig1)
...
fig2.suptitle("Panel 7B: Top 10 SHAP Gene Expression Across Conditions (GSE186476)", fontsize=get_font_size("title"), fontweight="bold")
fig2.tight_layout()
fig2.savefig(os.path.join(OUT_DIR, "Panel6B_violin.pdf"), bbox_inches="tight", dpi=300)
fig2.savefig(os.path.join(OUT_DIR, "Panel6B_violin.png"), bbox_inches="tight", dpi=300)
plt.close(fig2)
...
if has_spatial:
    try:
        ...
        fig_width, fig_height = get_figure_size(n_panels=12, layout='double')
        ...
        fig3.suptitle(
            "Panel 7B: Spatial Transcriptomics — Spots UMAP (GSM6021876, Lupus)\n"
            "Top 10 SHAP genes (Figure 5D): " + ", ".join(TOP10_GENES),
            fontsize=get_font_size("title"), fontweight="bold"
        )
        fig3.savefig(os.path.join(OUT_DIR, "Panel6B_spatial.pdf"), bbox_inches="tight", dpi=300)
        fig3.savefig(os.path.join(OUT_DIR, "Panel6B_spatial.png"), bbox_inches="tight", dpi=300)
        plt.close(fig3)
