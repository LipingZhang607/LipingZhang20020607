...
# -*- coding: utf-8 -*-
"""
Panel 6B: GSE186476 空间转录组验证 - GAT基因版本
- 定位GAT Top10标记基因在SLE病变组织中的空间分布
- 探究与免疫细胞浸润区域的共定位关系
...
OUT_DIR   = "/home/h3033/statics/GEO_data/GSE/figure6/output/panel6B_GAT"
...
# Top 10 GAT 基因（空间转录组验证）
GAT_FILE = "/home/h3033/statics/GEO_data/GSE/figure5/output/figure5C/feature_importance.csv"
_gat_df   = pd.read_csv(GAT_FILE)
TOP10_GENES = _gat_df.sort_values("importance_combined", ascending=False)["feature"].head(10).tolist()
print("Top10 GAT genes:", TOP10_GENES)
...
# ── 图1: scRNA-seq UMAP（条件 + 免疫评分 + Top10 GAT基因）─────────────────
# 布局 3×5：前3格=条件/单核/B细胞评分，后10格=Top10 GAT基因
fig1 = plt.figure(figsize=get_figure_size(n_panels=12, layout="double"))
...
for cond in COND_ORDER:
    ...
    ax.scatter(adata_sc.obsm["X_umap"][idx, 0], adata_sc.obsm["X_umap"][idx, 1],
               c=CONDITION_COLORS[cond], s=get_marker_size("small"), alpha=0.4, label=cond, rasterized=True)
ax.set_title("Condition", fontsize=get_font_size("title"), fontweight="bold")
ax.set_xlabel("UMAP1"); ax.set_ylabel("UMAP2")
ax.legend(markerscale=5, frameon=False, fontsize=get_font_size("tick"))
ax.set_aspect("equal")
...
if score_key in adata_sc.obs:
    sc_plot = ax.scatter(adata_sc.obsm["X_umap"][:, 0], adata_sc.obsm["X_umap"][:, 1],
                         c=adata_sc.obs[score_key], cmap="Reds", s=get_marker_size("small"), alpha=0.5, rasterized=True)
    plt.colorbar(sc_plot, ax=ax, shrink=0.7)
ax.set_title("Monocyte/Macrophage Score", fontsize=get_font_size("title"), fontweight="bold")
ax.set_xlabel("UMAP1"); ax.set_ylabel("UMAP2")
ax.set_aspect("equal")
...
if score_key in adata_sc.obs:
    sc_plot = ax.scatter(adata_sc.obsm["X_umap"][:, 0], adata_sc.obsm["X_umap"][:, 1],
                         c=adata_sc.obs[score_key], cmap="Blues", s=get_marker_size("small"), alpha=0.5, rasterized=True)
    plt.colorbar(sc_plot, ax=ax, shrink=0.7)
ax.set_title("B Cell Score", fontsize=get_font_size("title"), fontweight="bold")
ax.set_xlabel("UMAP1"); ax.set_ylabel("UMAP2")
ax.set_aspect("equal")
...
for (row, col), gene in zip(gene_positions, TOP10_GENES):
    ...
    if gene in adata_sc.var_names:
        ...
        sc_plot = ax.scatter(adata_sc.obsm["X_umap"][:, 0], adata_sc.obsm["X_umap"][:, 1],
                             c=expr, cmap=cmap_gene, s=get_marker_size("small"), alpha=0.5, rasterized=True)
        plt.colorbar(sc_plot, ax=ax, shrink=0.7, label="log1p expr")
        ax.set_title(f"{gene} Expression", fontsize=get_font_size("title"), fontweight="bold")
    else:
        ax.text(0.5, 0.5, f"{gene}\n(not found)", ha="center", va="center", transform=ax.transAxes)
        ax.set_title(f"{gene}", fontsize=get_font_size("title"))
    ax.set_xlabel("UMAP1"); ax.set_ylabel("UMAP2")
    ax.set_aspect("equal")

fig1.suptitle("Panel 6B (scRNA-seq): Top 10 GAT Genes in GSE186476", fontsize=get_font_size("title"), fontweight="bold")
fig1.savefig(os.path.join(OUT_DIR, "Panel6B_GAT_scrna.pdf"), bbox_inches="tight", dpi=300)
fig1.savefig(os.path.join(OUT_DIR, "Panel6B_GAT_scrna.png"), bbox_inches="tight", dpi=300)
plt.close(fig1)

# ── 图2: Top10 GAT 基因在各条件下的表达分布 ─────────────────────────────────
from scipy.stats import mannwhitneyu
fig2, axes2 = plt.subplots(2, 5, figsize=get_figure_size(n_panels=12, layout="double"))
...
for i, gene in enumerate(TOP10_GENES):
    ...
    ax.set_xticklabels(COND_ORDER, rotation=20, ha="right", fontsize=get_font_size("tick"))
    ax.set_ylabel("log1p Expression")
    ax.set_title(f"{gene}", fontsize=get_font_size("title"), fontweight="bold")
    ...
    ax.plot([0, 2], [ymax*1.05, ymax*1.05], "k-", linewidth=get_line_width("plot"))
    label = "***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else "ns"
    ax.text(1, ymax*1.07, f"Healthy vs Lupus-Les\n{label} (p={p:.1e})",
            ha="center", fontsize=get_font_size("tick"))

fig2.suptitle("Panel 6B: Top 10 GAT Gene Expression Across Conditions (GSE186476)", fontsize=get_font_size("title"), fontweight="bold")
fig2.tight_layout()
fig2.savefig(os.path.join(OUT_DIR, "Panel6B_GAT_violin.pdf"), bbox_inches="tight", dpi=300)
fig2.savefig(os.path.join(OUT_DIR, "Panel6B_GAT_violin.png"), bbox_inches="tight", dpi=300)
plt.close(fig2)

# ── 图3: 空间转录组 spots UMAP（Top10 GAT 基因表达）────────────────────────
if has_spatial:
    try:
        # 组织图像解压（如需要）
        if not os.path.exists(spatial_img) and os.path.exists(spatial_img_gz):
        ...
        fig3 = plt.figure(figsize=get_figure_size(n_panels=12, layout="double"))
        gs3  = gridspec.GridSpec(3, 4, figure=fig3, hspace=0.4, wspace=0.35)
        ...
        ax.set_title("Tissue Image\n(GSM6021876, Lupus)", fontsize=get_font_size("title"), fontweight="bold")
        ax.axis("off")
        ...
        sc_plot = ax.scatter(umap_xy[:, 0], umap_xy[:, 1],
                             ...
                             s=get_marker_size("medium"), alpha=0.7, rasterized=True)
        plt.colorbar(sc_plot, ax=ax, shrink=0.7, label="Total counts")
        ax.set_title("Spots UMAP\n(Total counts)", fontsize=get_font_size("title"), fontweight="bold")
        ax.set_xlabel("UMAP1"); ax.set_ylabel("UMAP2")
        ax.set_aspect("equal")

        # ── Spots UMAP: Top10 GAT 基因（格2~11）────────────────────────────
        ...
        for (row, col), gene in zip(positions, TOP10_GENES):
            ...
            if gene in adata_sp.var_names:
                ...
                sc_plot = ax.scatter(umap_xy[:, 0], umap_xy[:, 1],
                                     c=expr, cmap="YlOrRd",
                                     vmin=0, vmax=vmax,
                                     s=get_marker_size("medium"), alpha=0.8, rasterized=True)
                plt.colorbar(sc_plot, ax=ax, shrink=0.7, label="log1p expr")
                ax.set_title(f"Spots UMAP\n{gene}", fontsize=get_font_size("title"), fontweight="bold")
            else:
                ax.text(0.5, 0.5, f"{gene}\n(not found)", ha="center", va="center",
                        transform=ax.transAxes, fontsize=get_font_size("title"))
                ax.set_title(f"{gene}", fontsize=get_font_size("title"))
            ax.set_xlabel("UMAP1"); ax.set_ylabel("UMAP2")
            ax.set_aspect("equal")
        ...
        fig3.suptitle(
            ...
            "Top 10 GAT genes (Figure 5C): " + ", ".join(TOP10_GENES),
            fontsize=get_font_size("title"), fontweight="bold"
        )
        fig3.savefig(os.path.join(OUT_DIR, "Panel6B_GAT_spatial.pdf"), bbox_inches="tight", dpi=300)
        fig3.savefig(os.path.join(OUT_DIR, "Panel6B_GAT_spatial.png"), bbox_inches="tight", dpi=300)
        plt.close(fig3)
        print("  ✓ Spots UMAP 图已保存（Top10 GAT genes）")
    except Exception as e:
        print(f"  [警告] 空间图绘制失败: {e}")
        import traceback; traceback.print_exc()

print(f"\n完成！输出: {OUT_DIR}/Panel6B_GAT_*.pdf / .png")
