...
# ============================================================
# Journal style: Genes & Diseases (KeAi / Elsevier)
#   - Halftone (color/grayscale photos, scatter): >= 300 dpi
#   - Combination (line + halftone)            : >= 500 dpi
#   - Pure line art                             : >= 1000 dpi
#   - Lettering >= 7 pt; subscript/superscript >= 6 pt
#   - Column widths: 1col 90 mm (3.54 in) / 1.5col 140 mm (5.51 in) / 2col 190 mm (7.48 in)
#   - Vector formats (PDF/EPS) preferred; embed fonts (Type 42); Arial
# ============================================================
import matplotlib

# Elsevier column widths (inches)
COL_1   = 3.54   # 90  mm — single column
COL_1_5 = 5.51   # 140 mm — 1.5 column
COL_2   = 7.48   # 190 mm — double column

# DPI tiers per Elsevier artwork policy
DPI_HALFTONE = 300
DPI_COMBO    = 500
DPI_LINE     = 1000
...
_JOURNAL_RC = {
    ...
    'pdf.fonttype':       42,   # TrueType, fully editable
    'ps.fonttype':        42,
    'svg.fonttype':       'none',
    'font.size':          7,
    'axes.titlesize':     8,
...
def _save_journal_figure(fig, base_path, art_type='line',
                         rasterize_scatter=True):
    """Save one figure as PDF (vector, with embedded raster scatter), TIFF (LZW),
    and PNG preview — meeting Genes & Diseases standards:

      - art_type='halftone' → 300 dpi (color/grayscale photos)
      - art_type='combo'    → 500 dpi (line + halftone)
      - art_type='line'     → 1000 dpi (default; pure line art, journal-strictest)

    Vector elements (text, axes, ticks, legends, lines, bar/pie shapes) remain
    fully editable vectors in the PDF. Only scatter PathCollections are
    rasterized inline at the chosen DPI — this keeps UMAPs <10 MB while still
    meeting the 1000 dpi line-art standard at print size.

    Also auto-applies aspect='equal' on UMAP axes so points are never stretched
    by panel geometry (preventing the "rectangular UMAP" tampering accusation
    risk).
    """
    from pathlib import Path as _P
    base = _P(str(base_path))
    base.parent.mkdir(parents=True, exist_ok=True)

    dpi = {'halftone': DPI_HALFTONE,
           'combo':    DPI_COMBO,
           'line':     DPI_LINE}.get(art_type, DPI_LINE)

    # Preserve UMAP point geometry AND box convention: force a square axes box
    # on any axes whose x/y label starts with "UMAP". scanpy labels its UMAP
    # axes UMAP1/UMAP2. set_box_aspect(1) is independent of data range, which
    # is the correct journal convention for UMAP figures.
    for ax in fig.get_axes():
        xl = ax.get_xlabel() or ''
        yl = ax.get_ylabel() or ''
        if xl.upper().startswith('UMAP') or yl.upper().startswith('UMAP'):
            ax.set_aspect('equal')
            ax.set_box_aspect(1)
    ...
    if rasterize_scatter:
        for ax in fig.get_axes():
            for coll in ax.collections:
                # Scanpy/scatter PathCollections — rasterize so PDF stays small
                # while keeping the embedded image at >= 1000 dpi at print size.
                try:
                    coll.set_rasterized(True)
                except Exception:
                    pass
            # Also rasterize imshow-based heatmaps for size, but keep their
            # axes/text vector
    ...
    _fix_fonts()
    # In the PDF backend, rasterized-artist resolution is driven by figure.dpi,
    # NOT by the savefig dpi argument. Pin figure.dpi to the target so the
    # embedded scatter image actually hits the requested dpi at print size.
    _orig_dpi = fig.get_dpi()
    fig.set_dpi(dpi)
    try:
        # Vector PDF (text/lines selectable; scatter+images rasterized inline @ dpi)
        fig.savefig(str(base) + '.pdf',
                    dpi=dpi, bbox_inches='tight')
        # TIFF with LZW compression — Genes & Diseases accepts TIFF
        fig.savefig(str(base) + '.tiff',
                    dpi=dpi, bbox_inches='tight', format='tiff',
...
# Plot QC metrics (2-column layout, scatter rasterized — combination art)
fig, axes = plt.subplots(2, 2, figsize=(COL_2, COL_2 * 0.85))
...
plt.tight_layout()
_save_journal_figure(fig, Config.FIG_DIR / 'QC_metrics')
plt.show()
...
# Plot Figure 4A: 1×3 (double column, combination art)
fig, axes = plt.subplots(1, 3, figsize=(COL_2, COL_2 / 3.3))
...
axes[2].text(1.5, y_top * 1.02, f'{sig}\np={p_hc_sle:.2e}', ha='center', fontsize=7)
...
plt.tight_layout()
_save_journal_figure(fig, Config.FIG_DIR / 'Figure4A_target_score')
plt.show()
print(f"✅ Figure 4A saved")

# ==================== 5. Figure 4B: Target-Enriched Cell Subpopulation ====================
print("\n" + "="*80)
print("5. Figure 4B: Target-Enriched Cell Subpopulation Identification")
print("="*80)
...
print(f"Number of target-enriched cells: {adata.obs['is_high_score'].sum():,} ({adata.obs['is_high_score'].mean()*100:.1f}%)")
...
fig_s1, ax = plt.subplots(figsize=(COL_1, COL_1 * 1.25))
...
ax.text(0.5, _y_top * 1.02, f'p = {_pval:.2e}  {_sig}', ha='center', fontsize=7)
ax.axhline(20, color=_THR_C, lw=1.2, linestyle='--', alpha=0.6, label='Threshold = 20%')
ax.set_xticks([0, 1])
ax.set_xticklabels(['HC', 'SLE'], fontsize=7)
ax.set_ylabel('% Target-enriched cells per Donor\n(Top 20% Target Activity Score)', fontsize=7)
ax.set_title('Target-enriched cell Threshold Justification', fontsize=8, fontweight='bold')
ax.legend(fontsize=7, frameon=False)
ax.spines[['top', 'right']].set_visible(False)

fig_s1.tight_layout()
_save_journal_figure(fig_s1, Config.FIG_DIR / 'Figure4_S1_threshold_justification')
plt.show()
print(f"✅ Figure4_S1_threshold_justification saved  (donor MWU p={_pval:.2e})")

# Extract target-enriched cells
high_score_adata = adata[adata.obs['is_high_score']].copy()

# Re-cluster target-enriched cells (adata.X is already log1p normalized, no need to repeat)
...
# Plot Figure 4B: 2×2
fig4b, axes4b = plt.subplots(2, 2, figsize=(COL_2, COL_2 * 0.95))

# B1 (0,0): Highlight target-enriched cells on full UMAP
adata.obs['highlight'] = adata.obs['is_high_score'].map({True: 'Target-enriched', False: 'Other'})
sc.pl.umap(adata, color='highlight', ax=axes4b[0, 0],
           palette={'Target-enriched': '#E64B35', 'Other': '#DDDDDD'},
           title='Target-enriched cells (Top 20% Score)',
           show=False)
_fix_fonts()  # Fix fonts after scanpy plot

# B2 (0,1): Target-enriched cells colored by cell type
...
sc.pl.umap(high_score_adata, color='Cell Type', ax=axes4b[0, 1],
           title='Target-enriched cells by Cell Type', show=False)
_fix_fonts()  # Fix fonts after scanpy plot
...
# B4 (1,1): Per-cell-type proportion of target-enriched cells in HC vs SLE
# For each major cell type: compute fraction of target-enriched cells among HC and SLE separately
...
axes4b[1, 1].set_ylabel('Target-enriched cell Fraction (%)')
axes4b[1, 1].set_title('Target-enriched cell Fraction per Cell Type\n(HC vs SLE)')
axes4b[1, 1].legend(frameon=False, fontsize=7)
axes4b[1, 1].spines[['top', 'right']].set_visible(False)

fig4b.suptitle('Figure 4B: Target-enriched cell Subpopulation — HC vs SLE', fontsize=9, fontweight='bold', y=1.01)
fig4b.tight_layout()
_save_journal_figure(fig4b, Config.FIG_DIR / 'Figure4B_high_score_subset')
plt.show()
...
# Plot boxplot
fig, ax = plt.subplots(figsize=(COL_1_5, COL_1_5 * 0.75))
...
ax.set_ylabel('Target-Enriched Cell Proportion (%)')
ax.set_title('Target-Enriched Cell Proportion in SLE vs HC')
...
plt.tight_layout()
_save_journal_figure(fig, Config.FIG_DIR / 'Figure4C_disease_specificity')
plt.show()
...
if len(significant_markers) >= 5:
    ...
    # ── Volcano plot ──────────────────────────────────────────────────────────
    fig_v, ax_v = plt.subplots(figsize=(COL_1_5, COL_1_5 * 0.9))
    ...
    ax_v.set_xlabel('logFC (target-enriched vs non-target-enriched)', fontsize=7)
    ax_v.set_ylabel('-log10(FDR)', fontsize=7)
    ax_v.set_title('Differential Expression: Target-enriched cells\n(pseudobulk, paired t-test, BH correction)',
                   fontsize=8, fontweight='bold')
    ax_v.legend(fontsize=7, frameon=False)
    ax_v.spines[['top', 'right']].set_visible(False)
    fig_v.tight_layout()
    _save_journal_figure(fig_v, Config.FIG_DIR / 'Figure4D_volcano')
    plt.show()
    print("✅ Figure 4D volcano saved")

    # ── Dotplot ───────────────────────────────────────────────────────────────
    fig_d, ax_d = plt.subplots(figsize=(COL_2, COL_2 * 0.5))
    ...
    _fix_fonts()  # Fix fonts after scanpy plot
    plt.tight_layout()
    _save_journal_figure(fig_d, Config.FIG_DIR / 'Figure4D_marker_genes')
    plt.show()
    print("✅ Figure 4D dotplot saved")

    # ── HC vs SLE pseudobulk DE for top target-enriched marker genes ────────────────
    # Shows whether the target-enriched-cell markers are also elevated in SLE vs HC at the bulk level
    print("\nComputing HC vs SLE pseudobulk DE for top target-enriched marker genes...")
    ...
    # Plot: bar chart logFC SLE vs HC for top target-enriched markers, color by FDR
    ...
    _row_h    = 0.30
    _fig_h    = max(4, _n_genes * _row_h + 1.0)
    fig_hs, ax_hs = plt.subplots(figsize=(COL_1_5, _fig_h))
    ...
    ax_hs.legend(handles=[Patch(color='#E64B35', label='FDR < 0.05'),
                 ...
                 frameon=False, fontsize=7, prop={'family': 'Arial'})
    ax_hs.set_xlabel('logFC (SLE vs HC)', fontsize=7)
    ax_hs.set_ylabel('Gene', fontsize=7)
    ax_hs.set_title('Top Target-Enriched-Cell Markers: SLE vs HC Expression\n'
                    '(pseudobulk, Welch t-test, BH FDR)',
                    fontsize=8, fontweight='bold')
    ax_hs.tick_params(axis='both', labelsize=7)
    for lbl in ax_hs.get_yticklabels() + ax_hs.get_xticklabels():
        lbl.set_fontname('Arial')
    ax_hs.spines[['top', 'right']].set_visible(False)

    fig_hs.tight_layout()
    _save_journal_figure(fig_hs, Config.FIG_DIR / 'Figure4D_HCvsSLE_markers')
    plt.show()
    ...
    try:
        ...
        if go_enrich.results is not None and len(go_enrich.results) > 0:
            ...
            if len(go_results) > 0:
                ...
                fig, ax = plt.subplots(figsize=(COL_2, COL_2 * 0.6))
                ...
                ax.set_yticklabels([t[:60] + '...' if len(t) > 60 else t for t in top_go['Term']], fontsize=7)
                plt.colorbar(scatter, ax=ax, label='Adjusted P-value')
                ax.set_xlabel('Gene Count', fontsize=7)
                ax.set_ylabel('GO Biological Process', fontsize=7)
                ax.set_title('Functional Enrichment of Target-enriched cell Markers (Enrichr)', fontsize=8)
                ax.invert_yaxis()

                plt.tight_layout()
                _save_journal_figure(fig, Config.FIG_DIR / 'Figure4E_GO_enrichment')
                plt.show()
        ...
        if gsea_res.res2d is not None and len(gsea_res.res2d) > 0:
            ...
            if len(gsea_results_sig) > 0:
                ...
                ax.set_yticklabels([t[:60] + '...' if len(t) > 60 else t for t in top_gsea['Term']], fontsize=7)
                ax.set_xlabel('Normalized Enrichment Score (NES)', fontsize=7)
                ax.set_ylabel('Gene Set', fontsize=7)
                ax.set_title('GSEA: Top Enriched Pathways in Target-Enriched Cells', fontsize=8)
                ...
                cbar.set_label('-log10(FDR q-val)', fontsize=7)

                plt.tight_layout()
                _save_journal_figure(fig, Config.FIG_DIR / 'Figure4E_GSEA_enrichment')
                plt.show()
...
# 1. Figure 4D: Top 50 target-enriched cell marker genes
top50_markers = significant_markers.head(50) if len(significant_markers) >= 50 else significant_markers
top50_markers.to_csv(Config.RESULTS_DIR / 'Figure4D_target_enriched_cell_markers_top50.csv', index=False)
print(f"✅ Top 50 target-enriched cell marker genes saved: {len(top50_markers)} genes")
print(f"   File: {Config.RESULTS_DIR / 'Figure4D_target_enriched_cell_markers_top50.csv'}")
...
# 3. Figure 4B: Target-enriched cell labels (add target_enriched_cell column)
adata.obs['target_enriched_cell'] = adata.obs['is_high_score']
target_enriched_cells_df = pd.DataFrame({
    'cell_barcode': adata.obs_names,
    'target_enriched_cell': adata.obs['target_enriched_cell'].values,
    'target_score': adata.obs['target_score'].values,
    'donor_id': adata.obs['donor_id'].values,
    'disease': adata.obs['disease'].values,
    'cell_type': adata.obs['author_cell_type'].values
})
target_enriched_cells_df.to_csv(Config.RESULTS_DIR / 'Figure4B_target_enriched_cell_labels.csv', index=False)
print(f"✅ Target-enriched cell labels saved: {(adata.obs['target_enriched_cell'] == True).sum()} target-enriched / {len(adata.obs)} total")
print(f"   File: {Config.RESULTS_DIR / 'Figure4B_target_enriched_cell_labels.csv'}")

# Re-save complete data with target_enriched_cell labels
adata.write(Config.PROCESSED_DIR / 'adata_final_with_target_enriched_labels.h5ad')
print(f"✅ Complete data with target-enriched cell labels saved")
print(f"   File: {Config.PROCESSED_DIR / 'adata_final_with_target_enriched_labels.h5ad'}")

print("\n" + "="*80)
print("Key Output Files Summary:")
print("="*80)
print(f"1. Top 50 target-enriched cell marker genes: Figure4D_target_enriched_cell_markers_top50.csv")
print(f"2. Compound target activity scores: Figure4A_target_activity_scores.csv")
print(f"3. Target-enriched cell labels: Figure4B_target_enriched_cell_labels.csv")
print(f"4. Complete data (with labels): adata_final_with_target_enriched_labels.h5ad")
print("="*80)

# ==================== 10. Figure 4F: Target-enriched cell Subtype Characterization ====================
print("\n" + "="*80)
print("10. Figure 4F: Target-enriched cell Subtype Characterization")
print("="*80)
...
target_enriched_adata = adata[adata.obs['is_high_score'] == True].copy()
target_enriched_obs   = target_enriched_adata.obs
all_obs         = adata.obs
...
sens_counts = target_enriched_obs['cell_type_label'].value_counts()
all_counts  = all_obs['cell_type_label'].value_counts()
df_comp = pd.DataFrame({'Target_enriched_cells': sens_counts, 'All_cells': all_counts}).fillna(0).astype(int)
df_comp['Target_enriched_pct']    = df_comp['Target_enriched_cells'] / df_comp['Target_enriched_cells'].sum() * 100
df_comp['Background_pct']   = df_comp['All_cells'] / df_comp['All_cells'].sum() * 100
df_comp['Enrichment_ratio'] = (df_comp['Target_enriched_pct'] + 1e-3) / (df_comp['Background_pct'] + 1e-3)
df_comp = df_comp.sort_values('Target_enriched_pct', ascending=False)

# ── B. Per-cell-type top marker identification within target-enriched cells ──────────
# For each cell type: compute mean log1p expression of top marker genes
# → find top-2 genes → create "CellType (GeneA+ GeneB+)" label
print("\nIdentifying top marker genes per cell type in target-enriched cells...")
MIN_CELLS = 10  # minimum target-enriched cells per cell type to include
...
for ct in df_comp.index:
    ct_mask  = target_enriched_obs['cell_type_label'] == ct
    n_cells  = ct_mask.sum()
    if n_cells < MIN_CELLS:
        continue
    ct_adata = target_enriched_adata[ct_mask.values]
...
target_enriched_adata.obs['detailed_subtype'] = (
    target_enriched_adata.obs['cell_type_label'].apply(make_detailed_label)
)
# propagate back to full adata for UMAP
adata.obs['detailed_subtype'] = 'Non-target-enriched'
adata.obs.loc[target_enriched_adata.obs_names, 'detailed_subtype'] = target_enriched_adata.obs['detailed_subtype'].values

# Save detailed subtype table
df_detail = target_enriched_obs[['cell_type_label', 'donor_id', 'disease', 'target_score']].copy()
df_detail['detailed_subtype'] = target_enriched_adata.obs['detailed_subtype'].values
df_detail.to_csv(Config.RESULTS_DIR / 'Figure4F_target_enriched_detailed_subtypes.csv')
df_comp.to_csv(Config.RESULTS_DIR / 'Figure4F_target_enriched_cell_subtype_composition.csv')

# ── C. Heatmap: cell type × top marker genes (target-enriched cells only) ───────────
...
fig, ax = plt.subplots(figsize=(min(COL_2, max(COL_1_5, len(gene_union) * 0.30)),
                                max(COL_1, len(ct_order) * 0.25)))
im = ax.imshow(heatmap_mat.values, aspect='auto', cmap='YlOrRd')
ax.set_xticks(range(len(gene_union)))
ax.set_xticklabels(gene_union, rotation=45, ha='right', fontsize=7)
ax.set_yticks(range(len(ct_order)))
ax.set_yticklabels(ct_order, fontsize=7)
plt.colorbar(im, ax=ax, shrink=0.6, label='Mean log1p expression\n(target-enriched cells)')
ax.set_title('Top Marker Gene Expression by Cell Type\n(Target-enriched cells Only)',
             fontsize=8, fontweight='bold')
plt.tight_layout()
_save_journal_figure(fig, Config.FIG_DIR / 'Figure4F_marker_heatmap_per_celltype')
plt.show()
print("✅ Figure 4F heatmap saved")

# ── D. Dotplot: % expressing × mean expression (scanpy, target-enriched cells) ──────
if len(ct_order) > 0 and len(gene_union) > 0:
    target_enriched_adata.obs['cell_type_label'] = pd.Categorical(
        target_enriched_adata.obs['cell_type_label'], categories=ct_order
    )
    sc.pl.dotplot(target_enriched_adata, gene_union,
                  groupby='cell_type_label',
                  standard_scale='var',
                  title='Marker Gene Expression in Target-enriched cell Subtypes\n(dot size = % expressing, color = scaled mean)',
                  show=False,
                  figsize=(min(COL_2, max(COL_1_5, len(gene_union) * 0.30)),
                           max(COL_1, len(ct_order) * 0.22)))
    _fix_fonts()  # Fix fonts after scanpy plot
    _save_journal_figure(plt.gcf(), Config.FIG_DIR / 'Figure4F_dotplot_target_enriched_subtypes')
    plt.show()
...
fig, axes  = plt.subplots(1, 3, figsize=(COL_2, COL_2 * 0.32))

# Pie
ax = axes[0]
wedges, _, autotexts = ax.pie(
    df_comp['Target_enriched_pct'],
    labels=None,
...
ax.legend(wedges,
          ...
          fontsize=7, frameon=False, title='Subtype (top genes)', title_fontsize=7)
ax.set_title('Target-enriched cell Subtype\nComposition', fontsize=8, fontweight='bold')

# Proportion bar
ax = axes[1]
x, w = np.arange(len(df_comp)), 0.35
ax.bar(x - w/2, df_comp['Target_enriched_pct'],  w, label='Target-enriched cells', color='#E64B35', alpha=0.85)
ax.bar(x + w/2, df_comp['Background_pct'], w, label='All cells (BG)',  color='#4DBBD5', alpha=0.85)
ax.set_xticks(x)
ax.set_xticklabels(df_comp.index, rotation=45, ha='right', fontsize=7)
ax.set_ylabel('Proportion (%)', fontsize=7)
ax.set_title('Target-Enriched vs Background\nCell Type Proportion', fontsize=8, fontweight='bold')
...
ax.set_xlabel('log2(Enrichment Ratio)', fontsize=7)
ax.set_title('Cell Type Enrichment in Target-enriched cells\n(annotated with top marker genes)',
             fontsize=8, fontweight='bold')
ax.tick_params(axis='y', labelsize=7)
ax.spines[['top', 'right']].set_visible(False)

fig.suptitle('Figure 4F: Target-enriched cell Subtype Composition & Marker Gene Annotation',
             fontsize=9, fontweight='bold', y=1.01)
plt.tight_layout()
_save_journal_figure(fig, Config.FIG_DIR / 'Figure4F_target_enriched_cell_subtypes')
plt.show()
print("✅ Figure 4F composition summary saved")

# ── F. UMAP: detailed subtype labels on target-enriched cells ───────────────────────
fig, axes = plt.subplots(1, 2, figsize=(COL_2, COL_2 * 0.45))

# Left: full UMAP, highlight target-enriched
adata.obs['Target-enriched'] = adata.obs['is_high_score'].map({True: 'Target-enriched', False: 'Other'})
sc.pl.umap(adata, color='Target-enriched',
           palette={'Target-enriched': '#E64B35', 'Other': '#DDDDDD'},
           size=3, alpha=0.5, title='Target-enriched cells on UMAP',
           ax=axes[0], show=False)
_fix_fonts()  # Fix fonts after scanpy plot

# Right: target-enriched cells only, colored by detailed subtype
sc.pl.umap(target_enriched_adata, color='detailed_subtype',
           title='Target-enriched cell Subtypes\n(CellType + top marker genes)',
           size=8, ax=axes[1], show=False)
_fix_fonts()  # Fix fonts after scanpy plot

plt.tight_layout()
_save_journal_figure(fig, Config.FIG_DIR / 'Figure4F_target_enriched_UMAP_overlay')
plt.show()
