...
sc.settings.set_figure_params(dpi=300, fontsize=7, facecolor="white")
...
fig_width, fig_height = get_figure_size(n_panels=8, layout='double')
...
ax1.set_xlabel("UMAP1", fontsize=get_font_size('label'))
ax1.set_ylabel("UMAP2", fontsize=get_font_size('label'))
ax1.legend(markerscale=5, frameon=False, fontsize=get_font_size('legend'))
...
cbar = plt.colorbar(sc_plot, ax=ax2, shrink=0.7)
cbar.set_label("Module Score", fontsize=get_font_size('label'))
cbar.ax.tick_params(labelsize=get_font_size('tick'))
ax2.set_title("UMAP — Sensitive Cell Module Score", fontsize=get_font_size('title'), fontweight="bold")
ax2.set_xlabel("UMAP1", fontsize=get_font_size('label'))
ax2.set_ylabel("UMAP2", fontsize=get_font_size('label'))
...
ax3.set_xlabel("UMAP1", fontsize=get_font_size('label'))
ax3.set_ylabel("UMAP2", fontsize=get_font_size('label'))
...
for xi, xj, p in [(0, 1, p_ctrl_inact), (0, 2, p_ctrl_act), (1, 2, p_inact_act)]:
    ...
    ax4.text((xi+xj)/2, y_top+0.02, sig_label(p), ha="center", fontsize=get_font_size('annotation'))

ax4.set_xticks([0, 1, 2])
ax4.set_xticklabels(DISEASE_ORDER, rotation=15, ha="right", fontsize=get_font_size('tick'))
ax4.set_ylabel("Module Score", fontsize=get_font_size('label'))
ax4.set_title("Module Score by Disease Status", fontsize=get_font_size('title'), fontweight="bold")
...
if prop_df is not None and len(groups_prop) > 0:
    ...
    bp = ax_box.boxplot(
        ...
        medianprops=dict(color="black", linewidth=get_line_width('plot')),
        whiskerprops=dict(linewidth=get_line_width('axis')),
        capprops=dict(linewidth=get_line_width('axis')),
        flierprops=dict(marker="o", markersize=get_marker_size('medium'), linestyle="none", alpha=0.5),
    )
    ...
    for i, (grp, vals) in enumerate(zip(present_groups, box_data)):
        ...
        ax_box.scatter(jitter, vals, s=get_marker_size('medium')*5, color=DISEASE_COLORS[grp],
    ...
    for level, (xi, xj, p_val) in enumerate(
        ...
        ax_box.text((xi + xj) / 2, y_top + h_step * 0.05,
                    _sig(p_val), ha="center", va="bottom", fontsize=get_font_size('annotation'))

    ax_box.set_xticks(positions)
    ax_box.set_xticklabels(present_groups, rotation=15, ha="right", fontsize=get_font_size('tick'))
    ax_box.set_ylabel("Sensitive cells (%)", fontsize=get_font_size('label'))
    ...
    ax_box.text(0.98, 0.02, "P: cell-level MWU",
                transform=ax_box.transAxes, fontsize=get_font_size('tick'),
    ...
    ax_box.text(0.5, 0.5, "No donor info available",
                ...
                fontsize=get_font_size('label'), color="gray")
    ax_box.set_title("Sensitive Cell Proportion", fontsize=get_font_size('title'), fontweight="bold")
...
bars = ax5.barh(range(len(ct_order)), ct_order["mean"].values, color=colors,
                edgecolor="gray", linewidth=get_line_width('axis'))
ax5.errorbar(
    ct_order["mean"].values, range(len(ct_order)),
    xerr=ct_order["std"].values, fmt="none", color="black",
    capsize=2, linewidth=get_line_width('plot')
)
ax5.set_yticks(range(len(ct_order)))
ax5.set_yticklabels(ct_order.index, fontsize=get_font_size('tick'))
ax5.axvline(0, color="gray", linestyle="--", linewidth=get_line_width('grid'))
ax5.set_xlabel("Mean Module Score", fontsize=get_font_size('label'))
ax5.set_title("Sensitive Cell Score by Cell Subtype (fine_cell_type)",
              fontsize=get_font_size('title'), fontweight="bold")

fig.suptitle("Panel 7A: Sensitive Cell Module Score Validation in GSE189050",
             fontsize=get_font_size('suptitle'), fontweight="bold", y=0.98)
